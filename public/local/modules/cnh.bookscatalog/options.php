...
if ((!empty($save) || !empty($restore)) && $request->isPost() && check_bitrix_sessid()) {
	...
	} elseif ($request->getPost('books_per_page') && ($request->getPost('books_per_page') > 0) && ($request->getPost('books_per_page') < 100000)) {
		Option::set(
			ADMIN_MODULE_NAME,
			"books_per_page",
			$request->getPost('books_per_page')
		);
		CAdminMessage::showMessage(array(
...
<form method="post" action="<?=sprintf('%s?mid=%s&lang=%s', $request->getRequestedPage(), urlencode($mid), LANGUAGE_ID)?>">
	...
	<tr>
		<td width="40%">
			<label for="books_per_page"><?=Loc::getMessage("REFERENCES_BOOKS_PER_PAGE") ?>:</label>
		<td width="60%">
			<input type="number"
				name="books_per_page"
				value="<?=Bitrix\Main\Text\String::htmlEncode(Option::get(ADMIN_MODULE_NAME, "books_per_page", 500));?>"/>
		</td>
	</tr>
